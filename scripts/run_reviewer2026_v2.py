...
def snapshot_named(model: torch.nn.Module) -> dict[str, torch.Tensor]:
    return {name: p.detach().float().clone() for name, p in model.named_parameters()}
...
def run_pair(base_payload: dict[str, Any], family: str, stale: dict[str, Any] | None, batches: list[Any], val_batches: list[Any], calibration_batch: Any, future_start: int | None = None, instrument: bool = False) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for k in range(1, H + 1):
        ...
        # Row-level cleanup happens after each persisted campaign row.
...
def comparison_summary(rows: list[dict[str, Any]], metric_summary: dict[str, Any]) -> None:
    ...
    scale = json.loads((OUT / "controlled_scale" / "summary.json").read_text(encoding="utf-8")) if (OUT / "controlled_scale" / "summary.json").exists() else {}
    long = json.loads((OUT / "long_training" / "summary.json").read_text(encoding="utf-8")) if (OUT / "long_training" / "summary.json").exists() else {}
    lines = [
        ...
        "## Controlled Scale",
        "",
        f"- status: {scale.get('status', 'not run')}",
        f"- rows: {scale.get('rows', 'not run')}",
        f"- task-nonrecovered rows: {scale.get('task_nonrecovered_h100', 'not run')}",
        "",
        "## Longer Training",
        "",
        f"- status: {long.get('status', 'not run')}",
        f"- rows: {long.get('rows', 'not run')}",
        f"- task-nonrecovered rows: {long.get('task_nonrecovered_h100', 'not run')}",
        "",
        "## Alternative Metrics",
        "",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--steps", default="provenance,validate,h100,mechanism,multi,post")
    ...
    if "post" in steps:
        post_metrics()
    if "controlled-scale" in steps:
        from reviewer2026_v2_heavy_campaigns import run_controlled_scale
        run_controlled_scale()
    if "long-training" in steps:
        from reviewer2026_v2_heavy_campaigns import run_long_training
        run_long_training()
