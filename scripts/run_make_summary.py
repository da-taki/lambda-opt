"""Experiment 11: paper-ready tables and review-experiment summary.

Reads every results/review_experiments/*.json produced by experiments 1-10 and emits:
  * results/review_experiments/tables_for_paper.md   (10 manuscript-ready tables)
  * results/review_experiments/REVIEW_EXPERIMENT_SUMMARY.md
...
Usage:
    .venv\\Scripts\\python.exe scripts/run_make_summary.py \
        --branch review-experiments --commit <hash> \
...
def main():
    ...
    p.add_argument("--branch", default="review-experiments")
    ...
    tables = {}
    keys = {}
    tbl_lines = ["# manuscript-ready tables", "",
                 "Auto-generated from `results/review_experiments/*.json`. " + C.ANALYZER_DISCLAIMER, ""]
    ...
    C.save_text(R / "tables_for_paper.md", "\n".join(tbl_lines))
    ...
    lines = ["# Supplementary review-experiment summary", "", C.ANALYZER_DISCLAIMER, "",
    ...
    lines += ["", "## 9. Tables to insert into manuscript", "",
              ...
              "`results/review_experiments/`. Fixed seeds are used throughout (quadratic 0-4, "
              "MNIST 0-2, big models seed 0+).", "",
              "## 5 most important numbers", ""]
    for k in five_key:
        lines.append(f"* {k}")
    C.save_text(R / "REVIEW_EXPERIMENT_SUMMARY.md", "\n".join(lines))
    print("Wrote tables_for_paper.md and REVIEW_EXPERIMENT_SUMMARY.md")
