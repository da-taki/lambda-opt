"""Shared infrastructure for the SNCS v4 reviewer-proofing experiments.
...
RESULTS_DIR = REPO_ROOT / "results" / "sncs_v4"
...
def backup_if_exists(path: Path) -> Optional[Path]:
    ...
    ``results/sncs_v4/`` is created fresh by these experiments, so there are no
    ...
    if not path.exists():
        return None
    if os.environ.get("SNCS_V4_BACKUP") == "1":
