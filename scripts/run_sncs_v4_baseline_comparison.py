"""SNCS v4 -- Experiment 1: Baseline comparison.
...
Usage (from repo root, with the project venv):
    .venv\\Scripts\\python.exe scripts/run_sncs_v4_baseline_comparison.py
    .venv\\Scripts\\python.exe scripts/run_sncs_v4_baseline_comparison.py --models quadratic mnist_mlp resnet18
"""
import argparse
import sys
import time
...
def write_markdown(path, rows, summary, args):
    lines = ["# Experiment 1 -- Baseline comparison", "",
             ...
             "abstain).", "",
             "Exact command:", "",
             "```", "python scripts/run_sncs_v4_baseline_comparison.py "
             f"--models {' '.join(sorted(set(r['model'] for r in rows)))} "
             f"--seeds {' '.join(str(s) for s in sorted(set(r['seed'] for r in rows)))}", "```", "",
